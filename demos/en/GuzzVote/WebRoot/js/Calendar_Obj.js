...
Calendar.addEvent = function(el, evname, func) {
	if (el.attachEvent) { // IE
		...
		//el["on" + evname] = func;
		el.setAttribute("on" + evname, func);
	}
};
...
//create a element, but do not append it to the parent

Calendar.createElementNotAppand = function(type) {
	var el = null;
	if (document.createElementNS) {
		// use the XHTML namespace; IE won't normally get here unless
		// _they_ "fix" the DOM2 implementation.
		...
		(function(){
			...
			function setValue(oSrcElement, nKeyCode){
				...
				if(!oSrcElement.beOnFocus || oSrcElement.value==""){
					if(!cal.bFlatDisplay){//???DialogCalendar?????????????????????????????????????????????????????????
						oSrcElement.setAttribute("value", (nKeyCode - 48));//.value = (nKeyCode - 48);
						//alert("inner: " + oSrcElement.value);
					}
			...
			function getNumberInput(ev){
				var oSrcElement = ev.currentTarget;
			...
			//modified: 2006-08-15 by ldr， 2006-09-12 by ldr 
			...
			//modified by ldr, 2006-08-15
			function makeTimeInput(className, init, range_start, range_end) {
				var part = Calendar.createElementNotAppand("input");
				part.name = className;
				//part.type = "text";
				part.setAttribute('type', "text");
				//part.setAttribute('name', className);
			...
			//added by liudaoru, 06-09-22
			function makeSubmit(value){
				var smt = Calendar.createElementNotAppand("span");
				var baseStyle = "cursor:pointer;padding:0px 4px;";
				smt.appendChild(document.createTextNode(value));

				smt.style.cssText = baseStyle + "border:1px solid #d9d3d0;background:#f9f3f0;color:#666;";
				
				smt.onclick = function(ev){
					var cal = window._dynarch_popupCalendar;
					(Calendar.is_ie) && (ev = window.event);
					Calendar.cellClick(cal.currentDateEl, ev);
				};
				smt.onmouseover = function(){
					this.style.cssText = baseStyle + "border:1px solid #000;background:#fff;color:#000;";
				}
				smt.onmouseout = function(){
					this.style.cssText = baseStyle + "border:1px solid #d9d3d0;background:#f9f3f0;color:#666;";
				}
				return smt;
			};
			//submit button, add by liudaoru
			var Smt = makeSubmit(Calendar._TT["SUBMIT"]);
			cell.appendChild(Smt);
		})();
	} else {
