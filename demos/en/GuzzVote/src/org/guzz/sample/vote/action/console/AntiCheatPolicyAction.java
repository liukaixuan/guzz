...
public class AntiCheatPolicyAction extends SimpleFormController {
	...
	protected ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws Exception {
		...
		if("extraProp".equalsIgnoreCase(policy.getPolicyImpl())){
			name = "1 " + policy.getLimitedField() + " in " + policy.getMaxLife() + " seconds can only vote " + policy.getAllowedCount() + " times" ;
		}else if("IP".equalsIgnoreCase(policy.getPolicyImpl())){
			name = "1 IP in " + policy.getMaxLife() + " seconds can only vote " + policy.getAllowedCount() + " times" ;
		}else if("cookie".equalsIgnoreCase(policy.getPolicyImpl())){
			name = "1 user in " + policy.getMaxLife() + " seconds can only vote " + policy.getAllowedCount() + " times" ;
		}
