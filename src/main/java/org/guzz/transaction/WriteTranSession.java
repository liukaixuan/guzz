...
public interface WriteTranSession extends TranSession {
	 ...
	 * 
	 * <br>
	 * The {@link WriteTranSession} from which the batcher is created, shares the same {@link java.sql.Connection} and
	 * javax.transaction.Transaction within the batcher. <br>
	 * The resources of the batcher will be released once the {@link WriteTranSession} is closed.
	 * 
	 * <p>
	 * use {@link Guzz#getTableCondition()} as the tableConditon if @param sql contains shadow table.
	 * 
	 * @param sql
	 *            CompiledSQL sql
	 ...
	 * 
	 * <br>
	 * the {@link WriteTranSession} from which the batcher is created, share the same {@link java.sql.Connection} and
	 * javax.transaction.Transaction with the batcher. <br>
	 * The resources of the batcher will be released once the {@link WriteTranSession} is closed.
	 * 
	 * @param sql
	 *            CompiledSQL sql
	 * @param tableCondition
	 *            the condition to shadow table. a SQLBatch can only be used for one table even if the domainClass is
	 *            the same.
	 * @exception DaoException
	 *                may raise database exception
	 */
	...
	public Object findObjectByPK(String businessName, Serializable pk, LockMode lockMode);

	public Object findObjectByPK(Class domainClass, Serializable pk, LockMode lockMode);

	public Object findObjectByPK(Class domainClass, int pk, LockMode lockMode);
	 ...
	 * 
	 * @param domainObject
	 *            要读取属性的对象。将根据此对象的主键进行读取。
	 * @param propName
	 *            属性名称。属性必须是lazy属性或{@link ColumnDataLoader}，否则报错。
	 * @throws DaoException
	 *             如果对象或者属性在数据库中不存在，抛出异常。
	 */
	 ...
	 * 
	 * @throws DaoException
	 *             throw DaoException if SQLException throwed in the rollback.
	 */
