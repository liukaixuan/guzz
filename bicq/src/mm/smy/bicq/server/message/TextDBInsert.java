...
public class TextDBInsert{
	...
	/**
	* 添加离线ServerTextMessage消息(该消息于TextMessage类似)。
	* 可以多次调用该函数，每次重新添加数据。使用PreparedStatement，在每次调用后不关闭连接。
	* 如果对同一消息调用多次，将会向数据库中插入重复纪录，执行后不改变 设置的ServerTextMessage的状态。
	...
	*/
	...
	/**
	* 设置一个ServerTextMessage，替代目前该类中的ServerTextMessage对象。
	* @param message 要插入数据库的ServerTextMessage，如果该message==null, 直接return ;
	*/
